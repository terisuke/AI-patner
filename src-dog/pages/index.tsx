...
export default function Home() {
  ...
  const [systemPrompt, setSystemPrompt] = useState(() => SYSTEM_PROMPT("きみ","ポチ"));
  ...
  const [voicevoxSpeaker, setVoicevoxSpeaker] = useState("3");
  ...
  const [characterName, setCharacterName] = useState("ポチ");
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      setSystemPrompt(() => SYSTEM_PROMPT(params.userName || "きみ",params.characterName || "ポチ"));
      ...
      setVoicevoxSpeaker(params.voicevoxSpeaker || "3");
      ...
      setCharacterName(params.characterName || "ポチ");
    ...
    const unsubscribe = auth.onAuthStateChanged((user) => {
      if (user) {
        ...
        if (previousRoute === "/login") {
          ...
          handleSendChat(`あなたは${characterName}という名前の犬です。おかえり、また来てくれてありがとうわん！から始まる文章を出力して、あなたから会話を開始してください。`
          , "assistant");
        } else if (previousRoute === "/signup") {
          console.log("User logged in via signup page, sending first chat message");
          handleSendChat(`あなたは${characterName}という名前の犬です。「こんにちは、私は${characterName}！あなたの名前は何ていうわん？「〜だよ」って言う形で教えてわん♪」という文章をそのまま出力して、あなたから会話を開始してください。`, "assistant");
        } else {
          console.log("Continuing the conversation as it is neither signup nor login");
          handleSendChat(`あなたは${characterName}という名前の犬です。やっほー！前も来てくれたわんね？から始まる文章を出力して、あなたから会話を開始してください。`, "assistant");
        }
      } else {
        setUserId(null);
      }
