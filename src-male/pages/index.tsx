...
import { MessageInputContainer } from "../../src/components/messageInputContainer";
import { Meta } from "../../src/components/meta";
import usePreviousRoute from '../../src/components/usePreviousRoute';
import VrmViewer from "../../src/components/vrmViewer";
import { AIService, AIServiceConfig, getAIChatResponseStream } from "../../src/features/chat/aiChatFactory";
import { DEFAULT_PARAM, KoeiroParam } from "../../src/features/constants/koeiroParam";
import { SYSTEM_PROMPT } from "../../src/features/constants/systemPromptConstants";
import {
  Message,
  Screenplay,
  textsToScreenplay,
} from "../../src/features/messages/messages";
import { speakCharacter } from "../../src/features/messages/speakCharacter";
import { ViewerContext } from "../../src/features/vrmViewer/viewerContext";
import { fetchAndProcessComments } from "../../src/features/youtube/youtubeComments";
import { saveChatLog } from "../../src/lib/firebase";
import "../../src/lib/i18n";
...
export default function Home() {
  ...
  const [systemPrompt, setSystemPrompt] = useState(() => SYSTEM_PROMPT("きみ","健一"));
  ...
  const [voicevoxSpeaker, setVoicevoxSpeaker] = useState("12");
  ...
  const [characterName, setCharacterName] = useState("健一");
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      setSystemPrompt(() => SYSTEM_PROMPT(params.userName || "きみ",params.characterName || "健一"));
      ...
      setVoicevoxSpeaker(params.voicevoxSpeaker || "12");
      setGoogleTtsType(params.googleTtsType || "en-US-Neural2-B");
      ...
      setCharacterName(params.characterName || "健一");
    ...
    const unsubscribe = auth.onAuthStateChanged((user) => {
      if (user) {
        ...
        if (previousRoute === "/login") {
          ...
          handleSendChat(`あなたは${characterName}という名前の男性パートナーです。おかえり、また来てくれてうれいよ！から始まる文章を出力して、あなたから会話を開始してください。`
          , "assistant");
        } else if (previousRoute === "/signup") {
          console.log("User logged in via signup page, sending first chat message");
          handleSendChat(`あなたは${characterName}という名前の男性パートナーです。「こんにちは、僕は${characterName}！君の名前は何ていうの？「〜だよ」って言う形で教えてくれないかい？」という文章をそのまま出力して、あなたから会話を開始してください。`, "assistant");
        } else {
          console.log("Continuing the conversation as it is neither signup nor login");
          handleSendChat(`あなたは${characterName}という名前の男性パートナーです。やあ、前も来てくれたよね？から始まる文章を出力して、あなたから会話を開始してください。`, "assistant");
        }
      } else {
        setUserId(null);
      }
